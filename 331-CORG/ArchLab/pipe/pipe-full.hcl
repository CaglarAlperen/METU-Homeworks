...
#
#
#  When I checked figure 4.52 (structure of PIPE) in th textbook I saw
#  that returning address of RET instruction is read in stage memory as 
#  m_valM and it is same with W_valM. Therefore I don't need to wait 
#  for write back stage to send returning address to f_pc. This improved
#  my program a bit.
#  CPE before: 10.14	CPE after: 10.07
#
#  Then I realized that I can do a similar but more meaningful improvement
#  on JXX instruction. Base version of pipeline checks mispredicted branch
#  in Memory stage but we can detect misprediction in Execution stage with
#  checking e_Cnd value. Thus I don't need to bubble two inscructions
#  fetched after misprediction. If I update f_pc as E_valA when E_icode
#  is mispredicted JXX, the next instruction will be fetched from correct
#  branch. This decreases bubble count to 1 from 2 and provides massive
#  performance. 
# 
#  But there was a bug in the implementation. It was failing when there is
#  a mispredicted branch followed by a ret instruction. When I inspected 
#  cycles in detail, I saw that when ret instruction was in Decode stage,
#  f_pred val was not updated according to fetched instruction length.
#  Since I could not see any stall in cycles I did not suspect it was 
#  stalling. After hours of inspection I decided that it was stalling
#  fetch stage because there was a ret instruction in Decode stage but
#  since I update f_pc from Execution stage it fetches next instruction
#  but not update f_valP value. Thus it turns back to mispredicted branch.
#
#  After that I added an extra condition to F_stall to detect if there is 
#  a ret in Decode stage and mispredicted branch in execution branch don't
#  stall.
#
#  I am not sure about the last part of my explanation why it was not 
#  working first but it makes my code error free in all cases.
#
#  CPE before: 9.69	CPE after: 9.00
#
####################################################################
...
bool F_stall =
	...
	IRET in { E_icode } || 
	(IRET == D_icode && !(E_icode == IJXX && !e_Cnd));
...
bool D_bubble =
	# Mispredicted branch
	# (M_icode == IJXX && !M_Cnd) && E_icode == INOP ||
	# Stalling at fetch while ret passes through pipeline
	# but not condition for a load/use hazard
	!(E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB }) &&
	   !(E_icode == IJXX && !e_Cnd) && IRET in { D_icode, E_icode };
