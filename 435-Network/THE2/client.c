...
{   
    write(sfd, (void *)pkt, sizeof(struct packet));
}
...
{
    for (;;) {
        peer_addr_len = sizeof(peer_addr); //SİL
        nread = read(recv_sfd, rcv_buf, BUF_SIZE);
        if (nread == -1)
            continue;               /* Ignore failed request */

        struct packet *p;
        p = (struct packet *)rcv_buf;
        rdt_recv(p);
    }
}
